...
$().ready( function() {
	...
	$selectAll.click( function() {
		...
		if ($this.prop("checked")) {
			...
			if ($enabledIds.filter(":checked").size() > 0) {
				...
				$actButton.removeClass("disabled");
				$contentRow.addClass("selected");
			} else {
				$deleteButton.addClass("disabled");
				$actButton.addClass("disabled");
			}
		} else {
	...
	$sort.click( function() {
		...
		if(document.getElementById('sortFlag') != null){
   ...
   $(".backtomenu").click(function(){
			if($("body",window.top.document).hasClass("guide-step")) return;
			$(".left .menu ul",window.top.document).empty();
			$(".right .main",window.top.document).attr("class","main");
			$(".right .main .main-app-list",window.top.document).hide();
			// setTimeout(function(){
				$(".right .main .main-menu-list",window.top.document).show();
				$(".intro",window.top.document).show();
				$(".nav_bar",window.top.document).show();
				$(".nav_bar",window.top.document).find(".indicator_sys, .indicator_app").removeClass("current");
				$(".nav_bar",window.top.document).find(".indicator_sys").addClass("current");
				$(".left .menu",window.top.document).hide();
			//},50000);
		});
});